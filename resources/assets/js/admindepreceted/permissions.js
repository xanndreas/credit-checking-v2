...
$(function () {
    ...
    $('.datatable-Permission tbody').on('click', 'td:not(:first-child, :last-child)', (event) => {
        let row = table.row(event.currentTarget).data();

        $('#submitAddPermission').attr('data-id', row.id);
        $('input[name="title"]').val(row.title);

        let canvasSelector = document.getElementById('offcanvasAddPermission')
        canvasSelector.addEventListener('hidden.bs.offcanvas', function () {
            $('#addNewPermissionForm').trigger("reset");
            $('#submitAddPermission').attr('data-id', null);
        });

        let bsOffCanvasAddPermission = new bootstrap.Offcanvas(canvasSelector)
        bsOffCanvasAddPermission.show();
    });

    $('#submitAddPermission').on('click', function () {
        ...
        if (savedIds === ''|| typeof savedIds === 'undefined') {
            ...
            savesForm.attr('action', "/admin/permissions").submit();
        } else {
            hiddenPut.prop('disabled', false);
            savesForm.attr('action', "/admin/permissions/" + savedIds).submit();
        }
    });
});

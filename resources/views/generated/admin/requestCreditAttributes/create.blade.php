...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.requestCreditAttribute.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.request-credit-attributes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="object_name">{{ trans('cruds.requestCreditAttribute.fields.object_name') }}</label>
                <input class="form-control {{ $errors->has('object_name') ? 'is-invalid' : '' }}" type="text" name="object_name" id="object_name" value="{{ old('object_name', '') }}" required>

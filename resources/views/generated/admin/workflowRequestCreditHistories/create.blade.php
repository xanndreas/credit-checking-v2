...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.workflowRequestCreditHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.workflow-request-credit-histories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workflow_request_credit') ? 'is-invalid' : '' }}" name="workflow_request_credit_id" id="workflow_request_credit_id" required>
                    @foreach($workflow_request_credits as $id => $entry)
                        <option value="{{ $id }}" {{ old('workflow_request_credit_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('actor') ? 'is-invalid' : '' }}" name="actor_id" id="actor_id" required>
                    @foreach($actors as $id => $entry)
                        <option value="{{ $id }}" {{ old('actor_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('actor'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('process_notes') ? 'is-invalid' : '' }}" type="text" name="process_notes" id="process_notes" value="{{ old('process_notes', '') }}">

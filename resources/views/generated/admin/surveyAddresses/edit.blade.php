...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.surveyAddress.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.survey-addresses.update", [$surveyAddress->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('request_credit') ? 'is-invalid' : '' }}" name="request_credit_id" id="request_credit_id" required>
                    @foreach($request_credits as $id => $entry)
                        <option value="{{ $id }}" {{ (old('request_credit_id') ? old('request_credit_id') : $surveyAddress->request_credit->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('address_type') ? 'is-invalid' : '' }}" name="address_type" id="address_type" required>
                    ...
                    @foreach(App\Models\SurveyAddress::ADDRESS_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('address_type', $surveyAddress->address_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('addresses') ? 'is-invalid' : '' }}" name="addresses" id="addresses">{{ old('addresses', $surveyAddress->addresses) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('surveyor') ? 'is-invalid' : '' }}" name="surveyor_id" id="surveyor_id" required>
                    @foreach($surveyors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('surveyor_id') ? old('surveyor_id') : $surveyAddress->surveyor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

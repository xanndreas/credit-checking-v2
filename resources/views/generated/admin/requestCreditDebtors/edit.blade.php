...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.requestCreditDebtor.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.request-credit-debtors.update", [$requestCreditDebtor->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('personel_type') ? 'is-invalid' : '' }}" name="personel_type" id="personel_type" required>
                    ...
                    @foreach(App\Models\RequestCreditDebtor::PERSONEL_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('personel_type', $requestCreditDebtor->personel_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $requestCreditDebtor->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('identity_type') ? 'is-invalid' : '' }}" name="identity_type" id="identity_type">
                    ...
                    @foreach(App\Models\RequestCreditDebtor::IDENTITY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('identity_type', $requestCreditDebtor->identity_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('identity_number') ? 'is-invalid' : '' }}" type="text" name="identity_number" id="identity_number" value="{{ old('identity_number', $requestCreditDebtor->identity_number) }}">

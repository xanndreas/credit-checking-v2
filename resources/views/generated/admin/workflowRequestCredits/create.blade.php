...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.workflowRequestCredit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.workflow-request-credits.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="request_credit_batch">{{ trans('cruds.workflowRequestCredit.fields.request_credit_batch') }}</label>
                <input class="form-control {{ $errors->has('request_credit_batch') ? 'is-invalid' : '' }}" type="text" name="request_credit_batch" id="request_credit_batch" value="{{ old('request_credit_batch', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('last_change_by') ? 'is-invalid' : '' }}" name="last_change_by_id" id="last_change_by_id">
                    @foreach($last_change_bies as $id => $entry)
                        <option value="{{ $id }}" {{ old('last_change_by_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('process_status') ? 'is-invalid' : '' }}" name="process_status_id" id="process_status_id" required>
                    @foreach($process_statuses as $id => $entry)
                        <option value="{{ $id }}" {{ old('process_status_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
